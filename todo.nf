...
process htoDemux{
  publishDir path: "$params.outdir/htoDemul"
  input:
...
process htodemul_visualisation{
  publishDir path: "$projectDir/graphs"
  ...
  output:
  path 'ridgePlot' optional true into outChannel
  path 'scatterPlot' optional true into outChannel
  path 'vlnPlot' optional true into outChannel
  path 'heatmapPlot' optional true into outChannel
  path 'tsnePlot' optional true into outChannel
...
//     path pre_processing.out.output_object
   //main:
    //   htoDemux(intermediate_object,params.quantile_hto, params.kfunc, params.nstarts, params.nsamples, params.htoDemuxOutPath, params.nameOutputFileHTO)
   //emit:
